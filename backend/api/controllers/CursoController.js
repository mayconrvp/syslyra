...
class CursoController {
  ...
  static async listarCursoPorId(req, res){
    ...
    try{
      const curso = await database.Cursos.findOne({ where: {id: Number(id)}})
  ...
  static async excluirCurso(req, res){
    let id = req.params.id;
    try{
      const curso = await database.Cursos.destroy({ where: {id: Number(id)}})
      return res.status(200).send({message: `Curso de Id ${id} deletada`});
    }catch(err){
      return res.status(500).json(err.message);
    }
  }
